...
class MenuDrawer extends HTMLElement {
  ...
  openMenuDrawer(summaryElement) {
    ...
    this.openDefaultDrawerSubmenus();
  }

  openDefaultDrawerSubmenus() {
    this.mainDetailsToggle.querySelectorAll('[data-default-open="true"]').forEach((details) => {
...
/* Luxury 2026 interactions */
document.addEventListener('DOMContentLoaded', () => {
  const ease = 'cubic-bezier(0.25, 0.8, 0.25, 1)';

  /* Magnetic hover for primary CTAs */
  const magneticButtons = document.querySelectorAll('.royalgifts-theme .product-form__submit, .royalgifts-theme .shopify-payment-button__button');
  magneticButtons.forEach((btn) => {
    ...
    btn.addEventListener('pointerleave', () => {
      ...
      window.setTimeout(() => (btn.style.transition = ''), 400);
    });
  });

  /* Title shimmer kick-off */
  document.querySelectorAll('.royalgifts-title').forEach((title) => {
  ...
  /* Tab controller */
  document.querySelectorAll('.royalgifts-tabs').forEach((tabsRoot) => {
    const buttons = tabsRoot.querySelectorAll('[data-royal-tab]');
    const panes = tabsRoot.querySelectorAll('[data-royal-pane]');
    const underline = tabsRoot.querySelector('.royalgifts-tab-underline');

    const moveUnderline = (btn) => {
      if (!underline || !btn) return;
      const idx = Number(btn.dataset.royalIndex || 0);
      underline.style.transform = `translateX(${idx * 100}%)`;
      underline.style.width = `${100 / buttons.length}%`;
    };
    ...
    buttons.forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        const target = tabsRoot.querySelector(`#${btn.dataset.royalTab}`);
        if (target) {
          target.classList.add('is-active');
          window.dispatchEvent(new Event('resize'));
          document.dispatchEvent(new CustomEvent('trustoo:reinit'));
          document.dispatchEvent(new CustomEvent('reviews:tab:open'));
        }
        moveUnderline(btn);
      });
    });

    moveUnderline(tabsRoot.querySelector('[data-royal-tab].is-active') || buttons[0]);
  });

  /* Discount code */
  let _rgDiscount = '';

  document.querySelectorAll('.rg-discount__btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      const wrap = btn.closest('.rg-discount');
      const input = wrap.querySelector('.rg-discount__input');
      const msg = wrap.querySelector('.rg-discount__msg');
      const code = input.value.trim();

      if (!code) {
        msg.textContent = 'Please enter a discount code.';
        msg.className = 'rg-discount__msg rg-discount__msg--error';
      ...
      // Sync all discount inputs
      document.querySelectorAll('.rg-discount__input').forEach((inp) => { inp.value = code; });

      msg.textContent = `Code "${code}" will be applied at checkout.`;
      msg.className = 'rg-discount__msg rg-discount__msg--success';
      msg.removeAttribute('hidden');
    });
  });

  // Intercept all checkout buttons to include the discount in the URL
  document.addEventListener('click', (e) => {
    const btn = e.target.closest('#CartDrawer-Checkout, #checkout, [name="checkout"]');
    if (!btn || !_rgDiscount) return;
    e.preventDefault();
    e.stopImmediatePropagation();
    window.location.href = `/checkout?discount=${encodeURIComponent(_rgDiscount)}`;
  }, true);
});
